...
class NeuralNetwork:
    ...
    def input_count(self) -> int:
        """Get the number of inputs in the neural network.
    ...
    def hidden_nodes_count(self) -> int:
        """Get the number of nodes in the network's hidden layer.
    ...
    def output_count(self) -> int:
        """Get the number of outputs in the neural network.
    ...
    def training_set_count(self) -> int:
        """Get the number of stored training sets.
    ...
    def predict(self, input) -> tuple[float]:
        """Perform prediction based on given input.
    ...
    def accuracy(self) -> float:
        """Get the accuracy of the network by using the stored training data.
    ...
    def print_results(self) -> None:
        """Print training results in the terminal."""
    ...
    def _randomize_training_order(self) -> None:
        """Randomize the order of the training sets for next epoch."""
        import random
        random.shuffle(self._training_order)

    def _feedforward(self, input: list[float]) -> None:
        """Perform feedforward to calculate new output for each node.
    ...
    def _backpropagate(self, reference: list[float]) -> None:
        """Perform backpropagation to calculate new error for each node.
    ...
    def _optimize(self, input: list[float], learning_rate: float) -> None:
        """Perform optimization by adjusting the network's parameters.
    ...
    def _average_error(self, input: list[float], reference: list[float]) -> float:
        """Calculate the average error for given training set.
...
def _subtract_lists(lst1: list[float], lst2: list[float]) -> list[float]:
    """Subtract the content of two lists and provides the difference in another list.
...
def _average(data: list[float]) -> float:
    """Get the average of numbers held by a list.
