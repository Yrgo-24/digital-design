...
// -----------------------------------------------------------------------------
static void dense_layer_dealloc(struct dense_layer* self)
{
    free(self->output);
    free(self->error);
    free(self->bias);
    double_vector_2d_delete(&(self->weights), self->node_count);
}
                                    ...
                                    const enum act_func act_func)
{
    ...
    if (!self) { return NULL; }
    self->output       = double_vector_new(node_count);
    self->error        = double_vector_new(node_count);
    self->bias         = double_vector_new(node_count);
    self->weights      = double_vector_2d_new(node_count, weight_count);
...
void dense_layer_print(const struct dense_layer* self, FILE* ostream)
{
    ...
    double_vector_2d_print(self->weights, self->node_count, self->weight_count, ostream, 0);
    fprintf(ostream, "--------------------------------------------------------------------------------\n\n");
}
